...
def main(argv):
	...
	vcf_writer1.write_record(generate_sv('1', 20000001, 'sv0', '1', 50000000, True, True, str(), True, 'sv1', '1|0', '2', 38, 140))
	vcf_writer1.write_record(generate_sv('1', 50000000, 'sv1', '1', 20000001, True, False, str(), True, 'sv0', '1|0', '2', 38, 130))
	...
	vcf_writer3.write_record(generate_sv('1', 20000001, 'sv0', '1', 50000000, True, True, str(), True, 'sv1', '1|0', '2', 21, 100))
	vcf_writer3.write_record(generate_sv('1', 50000000, 'sv1', '1', 20000001, True, False, str(), True, 'sv0', '1|0', '2', 21, 101))
	vcf_writer3.write_record(generate_sv('1', 60000000, 'sv2', '1', 80000001, True, False, str(), True, 'sv3', '1|0', '1', 37, 167))
	vcf_writer3.write_record(generate_sv('1', 80000001, 'sv3', '1', 60000000, True, True, str(), True, 'sv2', '1|0', '1', 37, 173))
...
def generate_sv(chrm, pos, rec_id, alt_chr, alt_pos, alt_ori, alt_rO, alt_cS, alt_wMA, info_mateid, gt, cnadj, bdp, dp):
	...
	calls = [vcf.model._Call(0, 'TUMOR', svCallData(gt,cnadj,bdp,dp)), vcf.model._Call(1, 'NORMAL', svCallData('0|0','1', 0, dp))]
...
class svCallData:
	def __init__(self, gt = '0|0', cnadj = '1', bdp = '0', dp = '100'):
		self.GT = gt
		self.CNADJ = cnadj
		self.BDP = str(bdp)
		self.DP = str(dp)
